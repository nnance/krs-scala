...
class UserApiSpec extends FlatSpec with Matchers {
  "find for id 2" should "have a name TestUser02" in new TestModule {
    val user = userApi.find(2)
    user.get.name should be("TestUser02")
  }

  "find for id 6" should "be undefined" in new TestModule {
    val user = userApi.find(6)
